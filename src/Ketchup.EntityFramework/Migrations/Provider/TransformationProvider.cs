namespace Ketchup.EntityFramework.Migrations.Provider {
	...
	public abstract class TransformationProvider : ITransformationProvider {
		...
		private ISchemaInfoProvider _schemaInfo;
		...
		public ISchemaInfoProvider SchemaInfo {
			get {
				if (_schemaInfo == null) {
					_schemaInfo = new DefaultSchemaInfoProvider(this);
				}
				return _schemaInfo;
			}
		...
		/// <summary>
		///   If set to true the current transaction will be
		///   closed and reopened for each migration.
		///   The default is true
		/// </summary>
		protected bool CloseConnectionOnCommit { get; set; }
		...
		public virtual Column GetColumnByName(string table, string columnName) {
			return Array.Find(GetColumns(table),
			                  delegate(Column column) { return column.Name == columnName; });
		}
		...
		public virtual int Insert(string table, string[] columns, string[] values) {
			return
				ExecuteNonQuery(String.Format("INSERT INTO {0} ({1}) VALUES ({2})", table, String.Join(", ", columns),
				                              String.Join(", ", QuoteValues(values))));
		}
		...
		public List<long> AppliedMigrations {
			get {
				if (_appliedMigrations == null) {
					...
					SchemaInfo.LoadMigrations(_appliedMigrations);
				}
